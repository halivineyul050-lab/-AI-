...
> - 本地地址：<http://127.0.0.1:4173/>；生产地址：<https://ontimo.cn/>
> - 本地数据库为 SQLite；生产数据库为 MariaDB 10.5（`mysql2`）。
> - 本文的历史工具/文章计数和早期接口测试结果不是线上实时数据。
...
- SQLite WAL
- 外键约束
- 版本化迁移（以 `backend/migrations/` 中实际文件为准）
- 投稿审核状态
- 管理操作审计日志
- 事件和跳转数据定期清理
...
测试数量和结果会随代码变化，不在此文固定记录。以当前检出的代码运行 `npm test` 为准。
...
当前后端属于**可直接运行的 Node.js 模块化单体**：

- 本地与测试技术栈：Node.js 22 + SQLite；生产技术栈：Node.js 22 + MariaDB 10.5
- 前后端同源部署
- 本地启动不需要 Docker；项目有 `mysql2` npm 依赖，用于生产 MariaDB 连接
- 已覆盖网站核心读写与数据统计链路

未来架构调整应以真实容量、功能需求和维护成本为依据；本项目当前生产部署已经采用 MariaDB，不需要仅为上线而迁移 PostgreSQL。

---

本文最后核对日期：2026-09-24。接口字段与功能以当前源码和自动化测试为准；线上域名为 `https://ontimo.cn/api/v1`，本地开发默认地址为 `http://127.0.0.1:4173/api/v1`。
