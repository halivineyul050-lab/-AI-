...
初始种子包含 28 个工具；当前本地数据库已扩充为 130 条工具记录，其中 129 条已发布，并已按 1,500–2,000 条目录容量改为服务端分页。第一阶段采用 Node.js 内置 HTTP 服务与 SQLite，可零 npm 依赖启动并完成真实持久化；API 契约、实体边界和字段命名按模块化单体设计，生产阶段可以替换为 NestJS + PostgreSQL。
